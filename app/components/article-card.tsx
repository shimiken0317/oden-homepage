...
export function ArticleCard({ article, index = 0 }: { article: PostSummary; index?: number }) {
  return <article className="article-card">
    ...
    <div className="card-body"><div className="card-meta"><Link href={`/categories/${article.categorySlug}`}>{article.category}</Link><span>{article.publishedAt.replaceAll("-", ".")}</span></div><h3><Link href={`/articles/${article.slug}`}>{article.title}</Link></h3><p>{article.excerpt}</p><div className="card-foot"><span>約{article.readingTime}分</span><Link href={`/articles/${article.slug}`} aria-label={`${article.title}を読む`}>↗</Link></div></div>
  </article>;
}
